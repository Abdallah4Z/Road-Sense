...
class TestProcessImageLabelPair:
    def test_missing_image(self, tmp_path):
        ...
        config = {"image_processing": {"target_size": [640, 480], "save_format": "png"},
                  "label_conversion": {"class_mapping": {"Car": 0}, "exclude_classes": []}}
        result = process_image_label_pair(
            tmp_path / "nonexistent.png", tmp_path / "nonexistent.txt",
            tmp_path / "out.png", tmp_path / "out.txt", config
        )
    ...
    def test_successful_processing(self, tmp_path):
        ...
        out_img.mkdir(parents=True); out_lbl.mkdir(parents=True)
        ...
        config = {"image_processing": {"target_size": [320, 240], "save_format": "png"},
                  "label_conversion": {"class_mapping": {"Car": 0}, "exclude_classes": [],
                                       "min_bbox_size": 0.0},
                  "output": {"create_yolo_yaml": False}}
        result = process_image_label_pair(
            img_dir / "test.png", lbl_dir / "test.txt",
            out_img / "test.png", out_lbl / "test.txt", config
        )
    ...
    def test_with_normalize(self, tmp_path):
        from src.data.preprocess_dataset import process_image_label_pair
        img_dir = tmp_path / "img"; lbl_dir = tmp_path / "lbl"
        out_img = tmp_path / "out" / "images"; out_lbl = tmp_path / "out" / "labels"
        img_dir.mkdir(parents=True); lbl_dir.mkdir(parents=True)
        out_img.mkdir(parents=True); out_lbl.mkdir(parents=True)
        ...
        config = {"image_processing": {"target_size": [320, 240], "save_format": "png",
                  ...
                  "output": {"create_yolo_yaml": False}}
        result = process_image_label_pair(
            img_dir / "t.png", lbl_dir / "t.txt",
            out_img / "t.png", out_lbl / "t.txt", config
        )
        assert result["success"] is True
    ...
    def test_exclude_class(self, tmp_path):
        ...
        config = {"image_processing": {"target_size": [320, 240], "save_format": "png"},
                  "label_conversion": {"class_mapping": {"Car": 0},
                                       "exclude_classes": ["DontCare"],
                                       "min_bbox_size": 0.0},
                  "output": {"create_yolo_yaml": False}}
        result = process_image_label_pair(
            img_dir / "t.png", lbl_dir / "t.txt",
...
class TestPreprocessDataset:
    def test_missing_img_dir(self, tmp_path):
        ...
        cfg.write_text("input: {raw_data_dir: raw, image_subdir: img, label_subdir: lbl}\n"
...
class TestCreateYoloConfig:
    def test_creates_yaml(self, tmp_path):
        ...
        proc_dir.mkdir()
        cfg = {"label_conversion": {"class_mapping": {"Car": 0}},
               "yolo_config": {"names": {"0": "Car"}},
               "output": {}}
