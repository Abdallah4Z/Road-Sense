...
class TestAugmentDataset:
    ...
    def test_skip_missing_labels(self, tmp_path):
        ...
        stats = augment_dataset(
            img_dir=str(img_dir), label_dir=str(lbl_dir),
            output_img_dir=str(out_img), output_label_dir=str(out_lbl),
        )
    ...
    def test_successful_augmentation(self, tmp_path):
        ...
        out_img = tmp_path / "out_img"; out_lbl = tmp_path / "out_lbl"
        for d in [img_dir, lbl_dir, out_img, out_lbl]:
            d.mkdir(parents=True)
        ...
        stats = augment_dataset(
            img_dir=str(img_dir), label_dir=str(lbl_dir),
            output_img_dir=str(out_img), output_label_dir=str(out_lbl),
            augmentations_per_image=1, preset="light",
        )
    ...
    def test_image_size_option(self, tmp_path):
        ...
        stats = augment_dataset(
            img_dir=str(img_dir), label_dir=str(lbl_dir),
            output_img_dir=str(out_img), output_label_dir=str(out_lbl),
            augmentations_per_image=1, image_size=(64, 128),
        )
...
class TestMain:
    ...
    def test_main_invalid_image_size(self):
        ...
        from src.data.augment_dataset import main
        test_args = ["prog", "--img-dir", "/x", "--label-dir", "/x",
                     "--output-img-dir", "/x", "--output-label-dir", "/x",
                     "--image-size", "invalid"]
